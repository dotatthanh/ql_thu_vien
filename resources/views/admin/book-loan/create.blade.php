...
<body>
    ...
    <div class="container mt-5">
        <h1 class="title-admin">Tạo đơn mượn sách</h1>
    </div>

    <div class="container-fluid mt-5">
        <form method="POST" action="{{ route('book_loans.store') }}">
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-3">
                    <input type="date" name="from_date" class="form-control" required onchange="handlDateChange()">
                </div>
                <label class="col-2 col-form-label">Đến ngày <span class="text-danger">(*)</span></label>
                <div class="col-3">
                    <input type="date" name="to_date" class="form-control" required onchange="handlDateChange()">
                </div>
            </div>
            @csrf
            <table class="table table-bordered table-striped mt-3">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="book_loans[0][book_id]" class="form-control select2" required
                                onchange="handleBookChange(this, 0)">
                                <option value=""></option>
                                @foreach ($books as $book)
                                    <option value="{{ $book->id }}" data-price="{{ $book->price }}"
                                        data-sale="{{ $book->sale }}">{{ $book->name }}
                                    </option>
                                @endforeach
                            </select>
                        </td>
                        <td class="text-center">
                            <input type="number" name="book_loans[0][quantity]" class="form-control" min="1"
                                onkeyup="totalMoney(0)" required>
                        </td>
                        <td class="text-center" id="price-0"></td>
            ...
            <div class="text-center">
                <button type="submit" class="btn btn-success mt-2">Tạo đơn</button>
            </div>
        </form>
    </div>
